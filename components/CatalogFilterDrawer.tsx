...
export default function CatalogFilterDrawer({
  ...
  return (
    ...
    >
      <aside
        className="absolute left-0 top-0 flex h-full w-[min(24rem,92vw)] flex-col border-r border-slate-700/80 bg-slate-950/95 shadow-2xl shadow-black/50 batik-surface"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          <section>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-200/80 batik-subtle">
              Pilih Kategori
            </p>
            <div className="mt-4 grid gap-2">
              <button
                ...
                className={[
                  'rounded-2xl border px-4 py-3 text-left transition batik-outline',
                  selectedCategoryId === 'all'
                    ? 'border-amber-300/60 bg-amber-300/10 text-slate-50'
                    : 'border-slate-800 bg-slate-900/70 text-slate-200 hover:border-amber-300/40 hover:bg-slate-900',
                ].join(' ')}
              >
          ...
          <section className="mt-8">
            ...
            <div className="mt-4 space-y-3">
              {SORT_OPTIONS.map((option) => (
                ...
                className={[
                  'w-full rounded-2xl border px-4 py-3 text-left transition batik-outline',
                  sortOption === option.value
                    ? 'border-amber-300/60 bg-amber-300/10'
                    : 'border-slate-800 bg-slate-900/70 hover:border-amber-300/40 hover:bg-slate-900',
                ].join(' ')}
              >
                <p className="text-sm font-semibold text-slate-50">{option.label}</p>
                <p className="mt-1 text-sm leading-6 text-slate-300 batik-subtle">
                  {option.description}
                </p>
              </button>
            ))}
          ...
          <section className="mt-8">
            ...
            <div className="mt-3 flex items-center justify-between text-sm font-semibold text-slate-100">
              <span className="batik-gold">Min {formatIDR(priceBounds.min)}</span>
              <span className="batik-gold">Max {formatIDR(priceBounds.max)}</span>
            </div>
            <div className="mt-4 space-y-3">
              <input
                type="range"
              ...
              />
              <div className="flex items-center justify-between text-xs font-semibold text-slate-300 batik-subtle">
                <span>Min {formatIDR(priceRange.min)}</span>
                <span>Max {formatIDR(priceRange.max)}</span>
              </div>
            </div>
            <p className="mt-2 text-xs text-slate-300 batik-subtle">
