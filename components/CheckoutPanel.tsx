...
export default function CheckoutPanel({
  ...
  return (
    <section
      id="checkout"
      className="rounded-2xl border border-slate-700 bg-slate-900/80 p-4 shadow-xl shadow-black/20"
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <label className="flex flex-col gap-1 text-sm text-slate-200">
          ...
          <input
            ...
            className="rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none ring-amber-400 transition focus:ring"
      ...
      <label className="mt-3 flex flex-col gap-1 text-sm text-slate-200">
        Alamat
        <textarea
          value={customer.address}
          onChange={(event) => onCustomerChange('address', event.target.value)}
          className="min-h-[90px] rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none ring-amber-400 transition focus:ring"
      ...
      <div className="mt-4 rounded-xl border border-slate-700 bg-slate-800/70 p-3">
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="rounded-lg border border-emerald-400/50 px-3 py-2 text-sm font-semibold text-emerald-200 transition hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        {(buyerLocation.status === 'denied' ||
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-[1fr_auto]">
            <input
              ...
              className="rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none ring-amber-400 transition focus:ring"
              placeholder="Tempel link Google Maps lokasi pembeli"
            />
            <button
              type="button"
              onClick={onManualLocationApply}
              className="rounded-lg border border-slate-500 px-3 py-2 text-sm font-semibold text-slate-100 transition hover:bg-slate-700"
            >
              Gunakan Link
            </button>
          </div>
      ...
      <button
        ...
        className="mt-4 w-full rounded-lg bg-emerald-500 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
      >
        Pesan via WhatsApp
      </button>
    </section>
